...
interface ThreeDSliderProps {
  ...
  /** Which card is centered on mount, as a 0-100 progress value (0 = first item, 100 = last). Defaults to 50 (middle item). */
...
const ThreeDSlider: React.FC<ThreeDSliderProps> = ({
  ...
  useEffect(() => {
    ...
    const startProgress = Math.max(0, Math.min(100, initialProgress));
  ...
  }, [items, speedWheel, speedDrag, initialProgress]);
